...
beforeEach(async () => {
  ...
  await app.createUser(getEmptySession(), "alice", "alice123", "0000000000");
  await app.createUser(getEmptySession(), "bob", "bob123", "1111111111");
});
...
describe("Create a user and log in", () => {
  it("should create a user and log in", async () => {
    ...
    const created = await app.createUser(session, "barish", "1234", "1234567890");
    assert(created.user);
  ...
  it("duplicate username should fail", async () => {
    ...
    const created = await app.createUser(session, "barish", "1234", "2222222222");
    assert(created.user);
    await assert.rejects(app.createUser(session, "barish", "1234", "2222222222"));
  });
